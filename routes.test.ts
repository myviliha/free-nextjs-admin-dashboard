...
/**
 * **Nothing is exempt, and there used to be one.** `/overlay-source` rendered the modal panels open so
 * the monorepo's `scripts/page-templates.mjs` had markup to place in the HTML edition; it was linked
 * from nowhere and waived here by name. Neither the emitter nor the page is in this repository, so the
 * assertion is the unconditional one. An exemption list on a reachability test is where a dead page
 * goes to look accounted for, so if this ever needs one again, the entry has to say why the page
 * exists at all.
 */
it("every page is reachable from the sidebar", () => {
  expect([...declared].filter((route) => !NAV_HREFS.includes(route))).toEqual([]);
});
...
it("the shared route list is exactly this app's routes", () => {
  ...
  expect([...FREE_ROUTES].sort()).toEqual([...declared].sort());
});
