...
/**
 * **A separate export from `metadata`, which is the framework's rule and not a style choice.** Next
 * moved `themeColor`, `colorScheme` and `viewport` out of `metadata` in 14: left there they are
 * silently ignored, so the tag never reaches the document and nothing tells you.
 *
 * Two colours, matched to `--background` in each theme, so the browser chrome on iOS and Android is
 * the same shade as the page rather than white above a dark dashboard. `color-scheme` is what makes
 * the native form controls and scrollbars follow suit.
 */
