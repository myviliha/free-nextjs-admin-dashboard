...
export function Brand({
  ...
  return (
    <span className="flex items-center gap-3">
      {/* `next/image` rather than a bare `<img>`: it carries the intrinsic size, so the row reserves
          the mark's space before the file lands, and the optimiser serves this 512px PNG at the size
          and format the requesting browser actually wants. `priority` because the mark is in the
          sidebar on every route, so it is never the thing that should be deferred. */}
