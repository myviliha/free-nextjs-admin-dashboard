...
export function CalendarScreen() {
  ...
  React.useEffect(() => {
    setStart((current) => current || todayISO());
    setEnd((current) => current || todayISO());
  }, []);
  ...
  /**
   * The seed events are dated relative to today, so they are built **after mount**.
   *
   * `new Date()` during render is two different instants on the server and in the browser, which is
   * the hydration defect this app has already been bitten by twice. An empty calendar for one frame
